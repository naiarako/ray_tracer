...
def ray_tracer_bilint(angle, xx, zz, x, dx, dz, nx, nz, neg=True):
    ...
    if neg:  # if True, initialize also the output for the plane wave with negative angle
        ind_col_neg_angle = np.zeros_like(ind_col)
...
def ray_tracer_pixel(angle, xx, zz, x, dx, dz, nx, nz, neg=True):
    ...
    if angle == 0:  # if angle==0, trivial length of rays in each pixel
        ...
        for i in range(nx * nz):  # loop through all virtual receivers
            ...
            else:  # else, length is dz in each pixel except for the one where the virtual receiver is located (dz/2)
                ind_col[pointer:pointer + niz_all[i]] = nix_all[i] + np.arange(niz_all[i]) * nx
                ind_row[pointer:pointer + niz_all[i]] = i
        ...
        for i in range(nx * nz):  # loop through all virtual receivers
            ...
            ind_sort = np.argsort(np.sqrt((inter_points[:, 0] - inter_points[0, 0]) ** 2 +
                                          (inter_points[:, 1] - inter_points[
                                              0, 1]) ** 2))  # indices of sorted array elements
...
@njit(cache=True)
def ray_tracer_div_pixel(x_rec, xx, zz, x, dx, dz, nx, nz, neg=True):
    """
    Straight-ray tracer for diverging waves with source at x_rec to virtual receivers at all grid points specified
    by 'xx' and 'zz'. A constant speed of sound in each pixel is assumed for model parametrization.
    :param x_rec: x-position of transmitting transducer element [scalar]
    ...
    :param nx: number of grid points in x direction [scalar]
    :param nz: number of grid points in z direction [scalar]
    :param neg: if True, it gives the output for rays starting from 'xel' and '-xel'. [boolean]
                if False, it only gives the output for rays starting from 'xel'
    :return: vals: numpy array for non-zero values of the forward operator relating traveltimes with the
    slowness of the medium. [numpy array]
             ind_row: numpy array indicating the row number of non-zero values of the forward operator [numpy array]
             ind_col: numpy array indicating the column number of non-zero values of the forward operator [numpy array]
    ...
    ind_col = np.zeros((nx * nz * 3 * nz,), dtype=np.int64)  # initialize output. Column indices
    ind_row = np.zeros_like(ind_col)  # row indices
    vals = np.zeros((nx * nz * 3 * nz,))  # corresponding values of the matrix

    if neg:  # if True, initialize also the output for rays coming from '-xel'
        ind_col_neg = np.zeros_like(ind_col)
        ind_row_neg = np.zeros_like(ind_col)
    ...
    for i in range(nx * nz):  # loop through all virtual receivers
        ...
        if neg:  # to compute output for -xel, reorder the row and column indices
            ind_glob_neg = nx * iz - ix + nx - 1  # corresponding indices in xx- or zz-like array
            ind_col_neg[pointer: pointer + len(ind_glob)] = ind_glob_neg
            ind_row_neg[pointer: pointer + len(ind_glob)] = 2 * int(np.floor(i / nx)) * nx + (nx - 1) - i

        pointer += len(ind_glob)  # update pointer

    if neg:  # build sparse forward operator for -xel
        return vals, ind_row, ind_col, ind_row_neg, ind_col_neg
    else:
        return vals, ind_row, ind_col, ind_row, ind_col  # dummy output for last two variables


@njit(cache=True)
def ray_tracer_div_bilint(x_rec, xx, zz, x, dx, dz, nx, nz, neg=True):
    """
    Straight-ray tracer for diverging waves with source at x_rec to virtual receivers at all grid points specified
    by 'xx' and 'zz'. Bilinear interpolation is used as basis for model parameterization. Note: model parameters are
    defined at the vertices of pixels and not at the center.
    ...
    if neg:  # if True, initialize also the output for rays starting from -xel
        ind_col_neg_angle = np.zeros_like(ind_col)
        ind_row_neg_angle = np.zeros_like(ind_col)
    ...
    for i in range(nx * nz):  # loop through all virtual receivers
        ...
        # number of intersections in z direction from (x_surf,0) to (xx, zz)
        niz = niz_all[i]
        ...
        # number of intersections in x direction from (x_surf,0) to (xx, zz)
        if angle < 0:
            nix = int(abs(nix_all[i] - np.floor((x_surf - x[0]) / dx))) + 1
        else:
            nix = int(abs(nix_all[i] - np.floor((x_surf - x[0]) / dx)))
